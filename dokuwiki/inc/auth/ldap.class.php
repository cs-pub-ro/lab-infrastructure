...
class auth_ldap extends auth_basic {
	...
	/*/ Begin hack
	if (strstr($info['dn'], 'ou=Asistent')) 
		$info['grps'][] = 'asistent';
	elseif (strstr($info['dn'], 'ou=Student'))
		$info['grps'][] = 'student';
	// End hack*/
	/* Hack v2 */
	if (is_array($this->cnf['assistants']))
	if (in_array($info['name'], $this->cnf['assistants']))
		$info['grps'][] = 'assistant';
	if (is_array($this->cnf['teachers']))
	if (in_array($info['name'], $this->cnf['teachers']))
		$info['grps'][] = 'teacher';	
	/* \Hack v2 */
        return $info;
    }
